...
public class window extends JFrame {
	...
	// -------------------------------------------------------------------------------------------------------------------------------------------COPY----------
	private List listTGI;
	private List listProcureurs;
	private List listCours;
	private List listTitresProc;
	private List listTitresJuge;
	private List listGreffiers;
	private List listServicesTGI;

	private ArrayList<Proxy> arrayListTGI;
	private ArrayList<Proxy> arrayListProcureurs;
	private ArrayList<Proxy> arrayListCours;
	private ArrayList<Proxy> arrayListTitresProc;
	private ArrayList<Proxy> arrayListTitresJuge;
	private ArrayList<Proxy> arrayListGreffiers;
	private ArrayList<Proxy> arrayListServicesTGI;
	...
	public window() {
		...
		listTGI = new List();
		arrayListTGI = new ArrayList<>();
		ResultSet res = (ResultSet) facade.proxy("tgi", 0);

		try {
			while (res.next()) {
				p = new Proxy(res.getInt("id_tgi"), res.getString("libelle"));
				arrayListTGI.add(p);
				listTGI.add(p.toString());
			}
		...
		listProcureurs = new List();
		arrayListProcureurs = new ArrayList<>();
		res = (ResultSet) facade.proxy("procureur", 0);

		try {
			while (res.next()) {
				p = new Proxy(res.getInt("id_proc"), res.getString("nom") + " "
						+ res.getString("prenom"));
				arrayListProcureurs.add(p);
				listProcureurs.add(p.toString());
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		...
		res = (ResultSet) facade.proxy("cours", 0);

		try {
			while (res.next()) {
				p = new Proxy(res.getInt("id_cour"), res.getString("nom_cours"));
				arrayListCours.add(p);
				listCours.add(p.toString());
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		...
		listTitresProc = new List();
		arrayListTitresProc = new ArrayList<>();
		res = (ResultSet) facade.proxy("titreProc", 0);

		try {
			while (res.next()) {
				p = new Proxy(res.getInt("id_titre"), res.getString("libelle"));
				arrayListTitresProc.add(p);
				listTitresProc.add(p.toString());
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		...
		listTitresJuge = new List();
		arrayListTitresJuge = new ArrayList<>();
		res = (ResultSet) facade.proxy("titreJuge", 0);

		try {
			while (res.next()) {
				p = new Proxy(res.getInt("id_titre"), res.getString("libelle_titre"));
				arrayListTitresJuge.add(p);
				listTitresJuge.add(p.toString());
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		...
		listGreffiers = new List();
		arrayListGreffiers = new ArrayList<>();
		res = (ResultSet) facade.proxy("greffier", 0);

		try {
			while (res.next()) {
				p = new Proxy(res.getInt("id_greffier"), res.getString("nom") + " "
						+ res.getString("prenom"));
				arrayListGreffiers.add(p);
				listGreffiers.add(p.toString());
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		...
		listServicesTGI = new List();
		arrayListServicesTGI = new ArrayList<>();
		res = (ResultSet) facade.proxy("service", 0);

		try {
			while (res.next()) {
				p = new Proxy(res.getInt("id_service"), res.getString("libelle"));
				arrayListServicesTGI.add(p);
				listServicesTGI.add(p.toString());
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		...
		btnSupprimerPersonnes.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int confirm = JOptionPane.showConfirmDialog(panelEnqueteur,
						"Etes-vous sur?");
		...
		tabbedPanelEnqueteur.addTab("Titres Enqueteurs", null, panelTitresEnq,
				null);
		...
		final JPanel panelTGI = new JPanel();
		...
		btnSupprimerTGI.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int confirm = JOptionPane.showConfirmDialog(panelTGI,
						"Etes-vous sur?");
				if (confirm == JOptionPane.YES_OPTION) {
					final TGIDB j = (TGIDB) facade.create("tgi", null);
					j.load(arrayListTGI.get(listTGI.getSelectedIndex())
							.getIdBase());
					j.supprimer();
					refreshPanelJuges();
				}
		...
		tabbedPanelInstJuridique.addTab("Procureurs", null, panelProcureur,
				null);
		panelProcureur.setLayout(new BorderLayout(0, 0));

		
		panelProcureur.add(listProcureurs, BorderLayout.CENTER);
		...
		btnConsulterProcureur.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				new ConsulterProcureur(arrayListProcureurs.get(listProcureurs
						.getSelectedIndex()));
			}
		});
		...
		btnModifierProcureur.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				new ModifierProcureur(listProcureurs.getSelectedItem());
			}
		});
		...
		btnSupprimerProcureur.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int confirm = JOptionPane.showConfirmDialog(panelProcureur,
						"Etes-vous sur?");
				if (confirm == JOptionPane.YES_OPTION) {
					final procDB j = (procDB) facade.create("procureur", null);
					j.load(arrayListProcureurs.get(listProcureurs.getSelectedIndex())
							.getIdBase());
					j.supprimer();
					refreshPanelProcureur();
				}
			}
		});
		...
		listJuges = new List(1, false);

		panelJuges.add(listJuges, BorderLayout.CENTER);

		arrayListJuge = new ArrayList<Proxy>();
		res = (ResultSet) facade.proxy("juge", 0);
		try {
			while (res.next()) {
				p = new Proxy(res.getInt("id_juge"), res.getString("nom") + " "
						+ res.getString("prenom"));
				arrayListJuge.add(p);
		...
		btnConsulterJuges.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				new ConsulterJuge(arrayListJuge.get(listJuges
						.getSelectedIndex()));
			}
		});
		...
		btnModifierJuges.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				new ModifierJuge(
						arrayListJuge.get(listJuges.getSelectedIndex()),
						(window) panelPersonnes.getParent().getParent()
		...
		btnAjouterJuges.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				new AjouterJuge((window) panelPersonnes.getParent().getParent()
		...
		btnSupprimerJuges.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int confirm = JOptionPane.showConfirmDialog(panelJuges,
						"Etes-vous sur?");
				if (confirm == JOptionPane.YES_OPTION) {
					final JugeDB j = (JugeDB) facade.create("juge", null);
					j.load(arrayListJuge.get(listJuges.getSelectedIndex())
							.getIdBase());
					j.supprimer();
					refreshPanelJuges();
				}
		...
		tabbedPanelInstJuridique.addTab("Cours d'appel", null, panelCoursAppel,
				null);
		...
		JPanel panelTitreProc = new JPanel();
		tabbedPanelInstJuridique.addTab("Titres", null, panelTitreProc, null);
		panelTitreProc.setLayout(new BorderLayout(0, 0));

		JPanel panel_34 = new JPanel();
		panelTitreProc.add(panel_34, BorderLayout.SOUTH);
		...
		panelTitreProc.add(listTitres, BorderLayout.CENTER);

		final JPanel panelGreffiers = new JPanel();
		tabbedPanelInstJuridique
				.addTab("Greffiers", null, panelGreffiers, null);
		...
		btnSupprimerGreffiers.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int confirm = JOptionPane.showConfirmDialog(panelGreffiers,
						"Etes-vous sur?");
				if (confirm == JOptionPane.YES_OPTION) {
					// TODO Delete
				}
		...
		tabbedPanelInstJuridique.addTab("Services TGI", null, panelServicesTGI,
				null);
		...
		tabbedPanelGestionAffaire.addTab("Informations generales", null,
				panelInfosGenerales, null);
		...
		tabbedPanelGestionAffaire.addTab("Types de mission", null,
				panelTypesMissions, null);
		...
		tabbedPanelGestionAffaire.addTab("Types d'objets", null,
				panelTypesObjets, null);
		...
		tabbedPanelGestionAffaire.addTab("Materiels/Logiciels", null,
				panelMaterielsLogiciels, null);
		panelMaterielsLogiciels.setLayout(new BorderLayout(0, 0));

		List listMaterielsLogiciels = new List();
		panelMaterielsLogiciels
				.add(listMaterielsLogiciels, BorderLayout.CENTER);
		...
		tabbedPanelGestionAffaire.addTab("Tarification", null,
				panelTarifications, null);
	...
	public void refreshPanelProcureur() {
		listProcureurs.removeAll();
		arrayListProcureurs.clear();
		ResultSet res = (ResultSet) facade.proxy("procureur", 0);
		Proxy p;
		try {
			while (res.next()) {
				p = new Proxy(res.getInt("id_proc"), res.getString("nom") + " "
	...
	public void refreshPanelCour() {
		listCours.removeAll();
		arrayListCours.clear();
		ResultSet res = (ResultSet) facade.proxy("cours", 0);
		Proxy p;
		try {
			while (res.next()) {
				p = new Proxy(res.getInt("id_cour"), res.getString("nom_cours"));
	...
	public void refreshPanelTitre() {
		listTitresProc.removeAll();
		arrayListTitresProc.clear();
		ResultSet res = (ResultSet) facade.proxy("titreProc", 0);
		Proxy p;
		try {
			while (res.next()) {
				p = new Proxy(res.getInt("id_titre"), res.getString("libelle"));
	...
	public void refreshPanelGreffier() {
		listGreffiers.removeAll();
		arrayListGreffiers.clear();
		ResultSet res = (ResultSet) facade.proxy("greffier", 0);
		Proxy p;
		try {
			while (res.next()) {
				p = new Proxy(res.getInt("id_greffier"), res.getString("nom") + " "
	...
	public void refreshPanelService() {
		listServicesTGI.removeAll();
		arrayListServicesTGI.clear();
		ResultSet res = (ResultSet) facade.proxy("titreProc", 0);
		Proxy p;
		try {
			while (res.next()) {
				p = new Proxy(res.getInt("service"), res.getString("libelle"));
				arrayListServicesTGI.add(p);
				listServicesTGI.add(p.toString());
			}
		} catch (SQLException e) {
